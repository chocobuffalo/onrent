...
export const AddMoreMachinesModal = ({
  ...
  useEffect(() => {
    ...
    if (bookingSession.startDate) params.set("start_date", bookingSession.startDate);
    if (bookingSession.endDate) params.set("end_date", bookingSession.endDate);

    if (bookingSession.location?.address) {
      params.set("region", bookingSession.location.address);
    } else if (bookingSession.location?.lat && bookingSession.location?.lng) {
      params.set("lat", String(bookingSession.location.lat));
      params.set("lon", String(bookingSession.location.lng));
    }

    const url = `${process.env.NEXT_PUBLIC_API_URL_ORIGIN}/api/catalog?${params.toString()}`;
    console.log("🟠 Modal fetch catálogo URL:", url);

    fetch(url)
      .then(res => res.json())
      .then(data => {
        console.log("🟢 Items recibidos en modal:", data);
        setItems(Array.isArray(data) ? data : []);
      })
      .catch(err => console.error("❌ Error cargando catálogo en modal:", err));
