    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4 py-20">
      <div className="bg-white rounded-[20px] shadow-lg w-full max-w-xl px-16 py-16">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 text-[12px] font-light text-gray-700">
          {/* Nomber del usuario */}
           <div>
            <label className="block mb-1">Nombre de Usuario <span className="text-red-500">*</span></label>
            <input
              type="text"
              {...register('name')}
          ...
          {/* Correo / Teléfono */}
          <div>
            <label className="block mb-1">Correo electrónico<span className="text-red-500">*</span></label>
            <input
              type="text"
              {...register('email')}
          ...
          <button
            ...
            disabled={!isValid}
          >
             {isLoading ?<ImSpinner8 color="#ffffff" size={20} className="animate-spin mx-auto" />: <span>Crear cuenta</span>}
        ...
        {/* Google Login */}
        <button onClick={() => signIn('google', { callbackUrl: "/dashboard/profile", })}  className="hover:bg-[#0f0f26] hover:text-white w-full flex cursor-pointer items-center justify-center gap-3 bg-white border px-6 py-4 rounded-md shadow-sm hover:shadow-md transition text-[12px] font-light">
